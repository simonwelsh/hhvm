...
class ReflectionProperty implements Reflector {
  ...
  public function __construct($cls, $name) {
    if (func_num_args() != 2) {
      // Warning is thrown by HHVM when < 2, but we still need to null out
      if (func_num_args() > 2) {
        trigger_error(__METHOD__ . '() expects exactly 2 parameters, ' .
          func_num_args() . ' given', E_WARNING);
      }
      return null;
    }
    if ($cls && $name) {
  ...
  public static function export($cls, $name, $ret=false) {
    if (func_num_args() != 2 && func_num_args() != 3) {
      // Warning is thrown by HHVM when < 2, but we still need to null out
      if (func_num_args() > 3) {
        trigger_error(__METHOD__ . '() expects at most 3 parameters, ' .
          func_num_args() . ' given', E_WARNING);
      }
      return null;
    }
  ...
  public function getName() {
    if (func_num_args() != 0) {
      trigger_error(__METHOD__ . '() expects exactly 0 parameters, ' .
        func_num_args() . ' given', E_WARNING);
      return null;
    }
    return $this->info['name'];
  ...
  public function getValue($obj = null) {
    if (!$this->forceAccessible && !$this->isPublic()) {
      $class = $this->cls->getName();
      throw new ReflectionException('Cannot access non-public member ' .
  ...
  public function setValue($obj = null, $value = null) {
    if (func_num_args() == 1 && $this->isStatic()) {
      $value = $obj;
      $obj = null;
      $num_args = 2;
    } else {
