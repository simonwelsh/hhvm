...
class DateTime {
   ...
   * @param object $interval - DateInterval object containing the time to add.
   * @return object - Returns the DateTime object for method chaining
   */
  <<__Native>>
  function add(DateInterval $interval): this;
   ...
   * @param object $timezone - DateTimeZone for the given time
   ...
   * @param object $datetime2 - DateTime object to compare agains
   * @param bool $absolute - Whether to return absolute difference
   * @return object - Returns a DateInterval object representing the distance
   * between two times
   */
  <<__Native>>
  function diff(DateTime $datetime2,
   ...
   * timezone offset.
   * @return int - Returns the timezone offset in seconds from UTC on success or
   * FALSE on failure.
   */
  <<__Native>>
  ...
  /* Procedural style DateTimeZone date_timezone_get ( DateTime $object ) Return
   * time zone relative to given DateTime.
   ...
   * Alter the timestamp of a DateTime object by incrementing or decrementing in
   ...
   * @return object - Returns the modified DateTime object or FALSE on failure.
   */
  <<__Native>>
  function modify(string $modify): mixed;

  /* Procedural style DateTime date_date_set ( DateTime $object , int $year ,
   * int $month , int $day ) Resets the current date of the DateTime object to a
   * different date.
   * @param int $year - DateTime object returned by date_create(). The function
   ...
   */
  <<__Native>>
  function setDate(int $year,
                   int $month,
                   int $day): object;

  /* Procedural style DateTime date_isodate_set ( DateTime $object , int $year ,
   * int $week [, int $day = 1 ] ) Set a date according to the ISO 8601 standard
   * - using weeks and day offsets rather than specific dates.
   * @param int $year - DateTime object returned by date_create(). The function
   * modifies this object.
   ...
   */
  <<__Native>>
  function setISODate(int $year,
                      int $week,
                      int $day = 1): mixed;

  /* Procedural style DateTime date_time_set ( DateTime $object , int $hour ,
   * int $minute [, int $second = 0 ] ) Resets the current time of the DateTime
   * object to a different time.
   * @param int $hour - DateTime object returned by date_create(). The function
   * modifies this object.
   ...
   */
  <<__Native>>
  function setTimestamp(int $unixtimestamp): self;

  /* Procedural style DateTime date_timezone_set ( DateTime $object ,
   * DateTimeZone $timezone )
   * @param object $timezone - DateTime object returned by date_create(). The
   * function modifies this object.
   * @return object - Returns the modified DateTime object or FALSE on failure.
   */
  <<__Native>>
   ...
   * @param object $interval - DateInterval object containing the time to
   * subtract.
   * @return object - Returns the DateTime object for method chaining
   */
  <<__Native>>
  function sub(DateInterval $interval): self;
...
class DateTimeZone {
  ...
  /* Procedural style string timezone_name_get ( void ) Returns the name of the
   * timezone.
  ...
  /* Procedural style int timezone_offset_get ( DateTimeZone $object , DateTime
   * $datetime ) This function returns the offset to GMT for the date/time
   * specified in the datetime parameter. The GMT offset is calculated with the
   * timezone information contained in the DateTimeZone object being used.
   * @param object $datetime - DateTimeZone object returned by timezone_open()
...
class DateInterval {
   ...
   * notice.
   * @return mixed - Number of years, months, days, hours, minutes, seconds, or
   * total days for DateInterval, or true/false for date inversion.
   */
  <<__Native>>
   ...
   */
  <<__Native>>
  function __set(mixed $member,
                 mixed $value): mixed;

  /* Sets up a DateInterval from the relative parts of the string
   * @param string $time - A date with relative parts. Specifically, the
   * relative formats supported by the parser used for strtotime() and DateTime
 ...
 * @return object
 */
function date_add(DateTime $datetime,
 ...
 * @return mixed - Returns a new DateTime instance or FALSE on failure.
 */
function date_create_from_format(mixed $format,
                                 mixed $time,
                                 ?DateTimeZone $timezone = null): mixed {
 ...
 */
<<__Native>>
function date_parse_from_format(string $format,
                                string $date): mixed;
 ...
 * @return mixed
 */
function date_create(mixed $time = "",
                     ?DateTimeZone $timezone = null): mixed {
  ...
  if (!$ret->__fromString(time, $timezone, null, false)) {
    return false;
  }
  return $ret
 ...
 * @return object
 */
function date_interval_create_from_date_string(mixed $time): DateInterval {
  return DateInterval::createFromDateString($time);
}

/* @param object $interval
 * @param string $format_spec
 * @return string
 */
function date_interval_format(DateInterval $interval,
                              mixed $format_spec): string {
 ...
 * @param int $day
 */
function date_isodate_set(DateTime $object,
                          mixed $year,
                          mixed $week,
                          mixed $day = 1): void {
  return $object->setISODate($year, $week, $day);
}
 ...
 * @return string
 */
function date_format(DateTime $datetime,
                     mixed $format): string {
 ...
 * @param string $modify
 */
function date_modify(DateTime $object,
                     mixed $modify): void {
 ...
 * @return object
 */
function date_sub(DateTime $datetime,
                  DateInterval $interval): DateTime {
...
function date_sunrise(int $timestamp,
                      int $format = 1,
                      float $latitude = get_date_default_latitude(),
                      float $longitude = get_date_default_longitude(),
                      float $zenith = get_date_default_sunrise_zenith(),
                      float $gmt_offset = get_date_default_gmt_offset()): mixed;
...
function date_sunset(int $timestamp,
                     ...
                     float $latitude = get_date_default_latitude(),
                     float $longitude = get_date_default_longitude(),
                     float $zenith = get_date_default_sunset_zenith(),
                     float $gmt_offset = get_date_default_gmt_offset()): mixed;
 ...
 * @return object
 */
function date_timestamp_set(DateTime $datetime,
                            mixed $timestamp): DateTime {
 ...
 * @param object $timezone
 */
function date_timezone_set(DateTime $object,
                           DateTimeZone $timezone): void {
...
function date(string $format,
              int $timestamp = TimeStamp::Current()): mixed;
 ...
 */
<<__Native>>
function getdate(int $timestamp = TimeStamp::Current()): array;
 ...
 */
<<__Native>>
function gmmktime(int $hour = INT_MAX,
                  int $minute = INT_MAX,
                  int $second = INT_MAX,
                  int $month = INT_MAX,
                  int $day = INT_MAX,
                  int $year = INT_MAX): mixed;
 ...
 */
<<__Native>>
function gmstrftime(string $format,
                    int $timestamp = TimeStamp::Current()): string;
 ...
 */
<<__Native>>
function localtime(int $timestamp = TimeStamp::Current(),
 ...
 */
<<__Native>>
function mktime(int $hour = INT_MAX,
                int $minute = INT_MAX,
                int $second = INT_MAX,
                int $month = INT_MAX,
                int $day = INT_MAX,
 ...
 */
<<__Native>>
function strftime(string $format,
                  int $timestamp = TimeStamp::Current()): mixed;
 ...
 */
<<__Native>>
function strtotime(string $input,
                   int $timestamp = TimeStamp::Current()): mixed;
 ...
 */
<<__Native>>
function timezone_location_get(object $timezone): array;
 ...
 */
<<__Native>>
function timezone_name_get(object $object): string;

/* @param string $timezone
 * @return object
 */
<<__Native>>
function timezone_open(string $timezone): object;

/* @param object $object
 * @return array
 */
<<__Native>>
function timezone_transitions_get(object $object): array;
